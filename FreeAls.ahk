...
checkPortalModi() {
    ...
    for name, pattern in modifiers {
        isFound := FindText(&X, &Y, 515 - 150000, 605 - 150000, 515 + 150000, 605 + 150000, 0, 0, pattern)
...
StartPortal(portal) {
    ...
    Sleep(500)
    FixClick(190, 219) ; click Portals Tab
    Sleep(200)
    FixClick(369, 183) ; focus search bar
    Sleep(300)
    SlowType(portal)
    Sleep(100)
    ...
    for portal in portalCoords {
        FixClick(portal.x, portal.y)
        Sleep(350)

        mod := checkPortalModi()
        if mod && ArrayHasValue(excludeModi, mod) {
            continue
        }

        FixClick(755, 529) ; open portal
        Sleep(250)
        ...
        while FindText(&X, &Y, x1, y1, x2, y2, 0, 0, PortalStart) {
            Loop 3
                FixClick(87, 463)
            Sleep(100)
        }
    ...
    if !foundValid {
        MsgBox("❌ No suitable portal found (all were excluded).")
    }
}
...
ConfirmModeSelection() {
    ...
    if (modeDDL.Text != "Portals")
        ...
        OpenExcGi.Visible := true
    
    for _, units in unitControls {
        for _, ctrlGroup in units {
            ctrlGroup.button.Enabled := (modeDDL.Text != "Infinity Castle")
        }
    }
}
...
SaveUnitLayout(*) {
    ...
    ; Validate mode and map
    mode := modeDDL.Text
    map := mapDDL.Text

    if (mode = "" || map = "") {
        MsgBox("Please select both a mode and a map before saving unit layout.")
        return
    }
    ...
    for page, units in unitControls {
        for unitNum, ctrl in units {
            ...
            try {
                ...
                screenX := (ObjHasOwnProp(ctrl, "screenX") && ctrl.screenX != "") ? ctrl.screenX : ""
                screenY := (ObjHasOwnProp(ctrl, "screenY") && ctrl.screenY != "") ? ctrl.screenY : ""

                IniWrite(screenX, file, section, prefix "_screenX")
                IniWrite(screenY, file, section, prefix "_screenY")
            } catch as e {
                MsgBox("Failed to save unit [" page ":" unitNum "]: " e.Message)
            }
        }
    }
...
LoadUnitLayout() {
    ...
    for page, units in unitControls {
        for unitNum, ctrl in units {
            ...
            try {
                enabled := IniRead(file, section, prefix "_enabled", "0")
                x := IniRead(file, section, prefix "_x", "")
                y := IniRead(file, section, prefix "_y", "")
                screenX := IniRead(file, section, prefix "_screenX", "")
                screenY := IniRead(file, section, prefix "_screenY", "")
                ...
                if (screenX != "" && screenY != "") {
                    ctrl.screenX := screenX
                    ctrl.screenY := screenY
                } else {
                    ctrl.screenX := x
                    ctrl.screenY := y
                }

                unitControls[page][unitNum] := ctrl
            } catch as e {
                MsgBox("Failed to load unit [" page ":" unitNum "]: " e.Message)
            }
        }
    }
}
